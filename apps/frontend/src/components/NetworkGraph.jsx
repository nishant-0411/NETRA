...
export default function NetworkGraph({ 
  ...
  return (
    <div className={`relative bg-white rounded-xl border border-slate-200/90 shadow-xs flex flex-col overflow-hidden ${isMini ? 'h-[440px]' : ''}`} style={!isMini ? { height } : {}}>
      {/* Top Controls Toolbar */}
      <div className="px-4 py-2.5 bg-slate-50 border-b border-slate-200 flex flex-wrap items-center justify-between gap-3 text-xs z-10 select-none">
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-8 pr-3 py-1.5 rounded-lg border border-slate-300 bg-white text-slate-800 placeholder-slate-400 focus:outline-hidden focus:ring-2 focus:ring-teal-500/30 focus:border-teal-600 text-xs w-52 sm:w-64"
            />
            {searchQuery && (
              <button 
          ...
          >
            {showNoise ? <Eye className="w-3.5 h-3.5 text-amber-600" /> : <EyeOff className="w-3.5 h-3.5 text-slate-400" />}
            <span>Noise / Unverified Leads</span>
            <span className={`text-[10px] px-1.5 py-0.2 rounded font-mono-code font-bold ${
              showNoise ? 'bg-amber-200 text-amber-900' : 'bg-slate-200 text-slate-600'
            }`}>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-slate-300 bg-white text-slate-700 hover:bg-slate-100 transition-colors text-xs font-medium shadow-2xs"
          ...
          <button
            ...
            className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg border text-xs font-medium transition-colors shadow-2xs ${
              physicsEnabled 
          ...
          {isMini && onExpand && (
            <button
              ...
              className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-teal-600 text-white hover:bg-teal-700 transition-colors text-xs font-semibold shadow-xs"
      ...
      <div className="px-4 py-2 bg-white border-t border-slate-200 flex items-center justify-between flex-wrap gap-2 text-xs select-none">
        ...
        <div className="flex items-center gap-2 flex-wrap text-xs">
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-2 py-0.8 rounded-md transition-all border ${
                  isSelected 
                    ...
                    : 'hover:bg-slate-100 border-slate-200/80 font-medium'
                }`}
          ...
          {activeLegendFilter && (
            <button
              ...
              className="text-[10px] font-mono-code font-bold text-teal-700 hover:underline px-1"
            >
              Reset Filter
            </button>
          )}
        ...
        {/* Edge relations hint */}
        <div className="hidden xl:flex items-center gap-2 text-[11px] text-slate-500 font-mono-code">
          <span className="flex items-center gap-1">
            <span className="w-3 h-0.5 bg-slate-900"></span> Solid: Verified
          </span>
          <span className="flex items-center gap-1">
            <span className="w-3 border-t border-dashed border-slate-500"></span> Dashed: Noise/Lead
          </span>
        </div>
      </div>
    </div>
