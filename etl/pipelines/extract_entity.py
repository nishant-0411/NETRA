...
def _extract_fallback_entities(text: str) -> List[dict]:
    """Lightweight fallback entity extraction."""
...
def _generate_fallback_queries(entities: List[dict]) -> List[dict]:
    ...
    mapping = {
        ...
        "Vehicle": (
            "vehicles",
            "data.result.rc_number"
        ),

        "Account": (
            "accounts",
            "data.result.account_number"
        ),

        "License": (
            "licenses",
            "data.result.license_number"
        ),
    ...
    for item in entities:
        ...
        # Location and Organization remain extracted intelligence but have no
        # corresponding master-data collection.
...
def extract_entities_and_descriptions(text: str) -> Dict[str, Any]:
    """
    Extract entities and descriptions using local Ollama Qwen3-VL.
    ...
    if llm:
        ...
        try:
            ...
            # The wrapper is configured with temperature=0 and num_predict=1024.
            # Let it apply those values rather than relying on RunnableSequence
            # keyword forwarding, which varies by LangChain version.
            llm_output = chain.invoke({"document_text": text})

            print("\n===== RAW QWEN RESPONSE =====")
            print(llm_output)
            print("===== END QWEN RESPONSE =====\n")

            if llm_output:

                # Handle ```json ... ``` and normal JSON
...
def generate_db_queries(
    ...
    """
    Generate database queries for all supported entity types.
    """
    ...
    if llm:
        ...
        try:
            ...
            logger.error(
                f"LLM query generation failed: {e}"
            )
